...
namespace HireMe.UI.Pages.Admin
{
    public partial class CompanyList : System.Web.UI.Page
    {
        ...
        {
            var userId = new Guid(e.CommandArgument.ToString());
            Response.Redirect("~/Pages/Admin/ViewCompany.aspx?UserId=" + userId);
        }
        ...
        private void FillGridView(string Name = "")
        {
            ...
            {
                var context = new HireMeEntities();
                var CompanyIds = context.UserTypes.Where(x => x.UserType_lookupId == 221).ToList().Select(x => x.aspnet_Users.UserId);
                CompanyIds.Count();
